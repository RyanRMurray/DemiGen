...
---------------------------------------------------------------------------------------------------
  ...
  getTileData fpath = do
    ...
    let tsyms  = map (justAttr "symmetry") xTiles
    let tweis  = map (justAttr "weight") xTiles
    let tData  = zip3 tnames tsyms tweis
    let tiles  = map mkTile tData
    --get neighbours
    let xNs    = elChildren $ findEl "neighbors" $ findEl "set" $ fromJust xml
    let ln     = map (justAttr "left") xNs
    let rn     = map (justAttr "right") xNs
    let nData  = zip ln rn
    let neighbors = map (mkNeighbors tnames) nData
    --get images
